        ...
        stage('Build Test APKs') {
            steps {
                script {
                    ...
                    sh './gradlew :app:assembleDhisDebug :app:assembleDhisDebugAndroidTest :form:assembleAndroidTest'
                }
            }
        }
        stage('Run tests') {
            parallel {
                ...
                stage('Deploy and Run UI Tests') {
                    environment {
                        BROWSERSTACK = credentials('android-browserstack')
                        app_apk = sh(returnStdout: true, script: 'find app/build/outputs/apk/dhis/debug -iname "*.apk"')
