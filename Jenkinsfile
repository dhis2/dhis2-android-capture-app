    ...
    stages {
        stage('Check for [skip ci]') {
            ...
            steps {
                script {
                    if (isTargetDevelop()) {
                        currentBuild.description = "CI skipped: develop runs on GitHub Actions"
                        echo "✅ Skipping CI because the target branch is 'develop'. CI for develop runs on GitHub Actions."
                    } else {
                        currentBuild.description = "CI skipped: '[skip ci]' found in the PR"
                        echo "✅ Skipping CI because '[skip ci]' was found in the PR title or description."
                    }
                }
            }
        }
