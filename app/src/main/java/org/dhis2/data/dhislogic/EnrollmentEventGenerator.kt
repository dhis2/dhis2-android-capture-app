...
class EnrollmentEventGenerator(
    ...
    fun generateEnrollmentEvents(
        ...
        generatorRepository.enrollment(enrollmentUid)?.let { enrollment ->
            autogeneratedEvents(enrollment, ownerOrgUnitUid)
            firstStageToOpen(enrollment)
        } ?: Pair(enrollmentUid, null)

    private fun autogeneratedEvents(
        enrollment: Enrollment,
        ownerOrgUnitUid: String?,
    ) {
        val autogeneratedEvents =
            generatorRepository.enrollmentAutogeneratedEvents(
                enrollment.uid(),
                enrollment.program()!!,
            )
        autogeneratedEvents.forEach { programStage ->
            generateScheduledEvent(enrollment, programStage, ownerOrgUnitUid)
        }
    }
    ...
    private fun firstStageToOpen(enrollment: Enrollment): Pair<String, String?> {
        ...
        val stageToOpen =
            if (program?.useFirstStageDuringRegistration() == true) {
    ...
    private fun checkIfEventExist(
        ...
        val eventUidToOpen =
            generatorRepository.eventUidInEnrollment(
                enrollment.uid(),
                programStage.uid(),
            )
    ...
    private fun generateScheduledEvent(
        enrollment: Enrollment,
        programStage: ProgramStage,
        ownerOrgUnitUid: String? = null,
    ) {
        try {
            val eventUid =
                generatorRepository.addEvent(
                    enrollment.uid(),
                    enrollment.program()!!,
                    programStage.uid(),
                    ownerOrgUnitUid ?: enrollment.organisationUnit()!!,
                )
