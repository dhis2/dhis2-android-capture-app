...
public final class RulesRepository {
    ...
    private static String getField(String field, String attribute) {
        return isEmpty(attribute) ? field : attribute;
    }
    ...
    private static String getField(String dataElement) {
    ...
    public static RuleAction create(ProgramRuleActionType actionType, String programStage, String section, String attribute,
        ...
        String field = getField(dataElement);
        ...
        switch (actionType) {
            ...
            case DISPLAYKEYVALUEPAIR:
                ...
                break;
            case HIDEFIELD:
                return RuleActionHideField.create(content, getField(field, attribute));
            case HIDESECTION:
                return RuleActionHideSection.create(section);
            case ASSIGN:
                return RuleActionAssign.create(content, getData(data), getField(field, attribute));
            case SHOWWARNING:
                return RuleActionShowWarning.create(content, data, getField(field, attribute));
            case WARNINGONCOMPLETE:
                return RuleActionWarningOnCompletion.create(content, data, getField(field, attribute));
            case SHOWERROR:
                return RuleActionShowError.create(content, data, getField(field, attribute));
            case ERRORONCOMPLETE:
                ...
                return RuleActionErrorOnCompletion.create(content, data, getField(field, attribute));
            case CREATEEVENT:
            ...
            case SETMANDATORYFIELD:
                return RuleActionSetMandatoryField.create(getField(field, attribute));
            case HIDEOPTION:
                return RuleActionHideOption.create(content, getField(field, attribute), option);
    ...
    private String getQuery(EventModel eventModel) {
        return eventModel.enrollment() == null ? QUERY_OTHER_EVENTS : QUERY_OTHER_EVENTS_ENROLLMENTS;
    }

    private String getUid(EventModel eventModel, ProgramModel programModel) {
        return eventModel.enrollment() == null ? programModel.uid() : eventModel.enrollment();
    }

    private String getUidToEvaluate(String eventUidToEvaluate) {
        return eventUidToEvaluate == null ? "" : eventUidToEvaluate;
    }
                    ...
                    return briteDatabase.createQuery(ProgramModel.TABLE, "SELECT Program.* FROM Program JOIN Event ON Event.program = Program.uid WHERE Event.uid = ? LIMIT 1", eventUidToEvaluate == null ? "" : eventUidToEvaluate)
                            .mapToOne(ProgramModel::create).flatMap(programModel ->
                                    briteDatabase.createQuery(EventModel.TABLE,
                                            getQuery(eventModel),
                                            getUid(eventModel, programModel),
                                            getUidToEvaluate(eventUidToEvaluate),
