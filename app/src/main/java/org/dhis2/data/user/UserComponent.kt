...
interface UserComponent {
    ...
    operator fun plus(programEventDetailModule: ProgramEventDetailModule): ProgramEventDetailComponent


    operator fun plus(searchTEModule: SearchTEModule): SearchTEComponent

    operator fun plus(dashboardModule: TeiDashboardModule): TeiDashboardComponent

    operator fun plus(qrModule: QrModule): QrComponent

    operator fun plus(qrModule: QrEventsWORegistrationModule): QrEventsWORegistrationComponent

    operator fun plus(dataDetailModule: TeiDataDetailModule): TeiDataDetailComponent

    operator fun plus(eventDetailModule: EventDetailModule): EventDetailComponent

    operator fun plus(teiProgramListModule: TeiProgramListModule): TeiProgramListComponent

    operator fun plus(enrollmentModule: FormModule): FormComponent

    operator fun plus(programModule: ProgramModule): ProgramComponent

    operator fun plus(eventInitialModule: EventInitialModule): EventInitialComponent

    operator fun plus(eventInitialModule: EventSummaryModule): EventSummaryComponent

    operator fun plus(syncManagerModule: SyncManagerModule): SyncManagerComponent

    operator fun plus(programStageSelectionModule: ProgramStageSelectionModule): ProgramStageSelectionComponent

    operator fun plus(qrReaderModule: QrReaderModule): QrReaderComponent

    operator fun plus(aboutModule: AboutModule): AboutComponent

    operator fun plus(dataSetDetailModel: DataSetDetailModule): DataSetDetailComponent

    operator fun plus(dataSetInitialModule: DataSetInitialModule): DataSetInitialComponent

    operator fun plus(dataSetTableModule: DataSetTableModule): DataSetTableComponent

    operator fun plus(reservedValueModule: ReservedValueModule): ReservedValueComponent

    operator fun plus(syncDataWorkerModule: SyncDataWorkerModule): SyncDataWorkerComponent

    operator fun plus(syncDataWorkerModule: SyncMetadataWorkerModule): SyncMetadataWorkerComponent

    operator fun plus(reservedValuesWorkerModule: ReservedValuesWorkerModule): ReservedValuesWorkerComponent

    operator fun plus(eventCaptureModule: EventCaptureModule): EventCaptureComponent

    operator fun plus(enrollmentModule: EnrollmentModule): EnrollmentComponent

    operator fun plus(nfcModule: NfcDataWriteModule): NfcDataWriteComponent
}
