...
public class EventCaptureRepositoryImpl implements EventCaptureContract.EventCaptureRepository {
    ...
    private static final String SELECT_SECTIONS = "SELECT\n" +
            ...
            "WHERE Event.uid = ?\n" +
            "AND " + EventModel.TABLE + "." + EventModel.Columns.STATE + NOT_EQUAL + "'" + State.TO_DELETE + "' ORDER BY ProgramStageSection.sortOrder";
    ...
    private StringBuilder getVariableList(List<String> variableList) {
    ...
    private HashMap<String, Rule> getRuleMap(List<Rule> ruleList, AtomicReference<String> selectedProgramUid) {
    ...
    private Flowable<List<Rule>> getRulesFor(String lastUpdatedElement) {
        ...
        return briteDatabase.createQuery(PROGRAM_STAGE_TABLE,
                ...
                .flatMap(programUid -> {
                    ...
                    StringBuilder st = getVariableList(variableList);
                    ...
                    HashMap<String, Rule> ruleMap = getRuleMap(ruleList, selectedProgramUid);
                    return (List<Rule>) ruleMap.values();
                }).toFlowable(BackpressureStrategy.LATEST);
    }
    ...
    private void updateEnrollment(String enrollmentUid) {
        String selectEnrollment = SELECT + ALL + FROM + EnrollmentModel.TABLE + WHERE +
                "uid = ? LIMIT 1;";
    ...
    public Observable<String> programStage() {
        return briteDatabase.createQuery(EventModel.TABLE, SELECT + ALL + FROM + EventModel.TABLE + WHERE + "Event.uid = ?", eventUid)
    ...
    private static final String QUERY_EVENT = SELECT + "Event.uid,\n" +
            ...
            "WHERE Event.uid = ?\n" +
            "AND " + EventModel.TABLE + "." + EventModel.Columns.STATE + NOT_EQUAL + "'" + State.TO_DELETE + "'\n" +
            "LIMIT 1;";
    ...
    private static final String QUERY_VALUES = "SELECT " +
            ...
            " WHERE event = ? AND value IS NOT NULL AND " + EventModel.TABLE + "." + EventModel.Columns.STATE + NOT_EQUAL + "'" + State.TO_DELETE + "';";
