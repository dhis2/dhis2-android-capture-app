...
//        loadDataElementRules(currentEvent);
            ...
            try (Cursor cursor = briteDatabase.query(OPTIONS, fieldViewModel.optionSet() == null ? "" : fieldViewModel.optionSet())) {
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    for (int i = 0; i < optionCount; i++) {
                        renderList.add(createFieldViewModel(cursor, optionCount, fieldViewModel, renderingType));
                        cursor.moveToNext();
                    }
                }
            }
    ...
    private FieldViewModel createFieldViewModel(Cursor cursor, int optionCount, FieldViewModel fieldViewModel,
                                ...
                                .switchMap(ruleEngine -> {
                                    if (isEmpty(lastUpdatedUid))
                                        ...
                                        return Flowable.just(dataElementRules.get(lastUpdatedUid) != null ? dataElementRules.get(lastUpdatedUid) : new ArrayList<Rule>())
                                                .map(rulesResult1 -> rulesResult1.isEmpty() ? trasformToRule(mandatoryRules) : rulesResult1)
                                                .flatMap(rulesResult2 -> Flowable.fromCallable(ruleEngine.evaluate(event, rulesResult2)));
                                })
                                .map(Result::success)
