...
public class EventCaptureRepositoryImpl implements EventCaptureContract.EventCaptureRepository {
    ...
    public Observable<List<Event>> getTEIEnrollmentEvents() {
        ...
        return d2.eventModule().events().byEnrollmentUid().eq(enrollment.uid())
                ...
                .filter(event -> {
                    if (Boolean.FALSE.equals(d2.programModule().programs().uid("WSGAb5XwJ3Y").blockingGet().ignoreOverdueEvents())) {
                        return event.status() != EventStatus.SCHEDULE || !event.dueDate().before(DateUtils.getInstance().getToday());
                    }
                    return true;
                })
                .toList()
