...
public class ProgramFragment extends FragmentGlobalAbstract implements ProgramContract.ProgramView, OrgUnitInterface {
    ...
    private void noSelectedDates(List<Date> selectedDates, SimpleDateFormat weeklyFormat) {
            ...
            if (currentPeriod != DAILY && currentPeriod != NONE) {
                new RxDateDialog(getAbstractActivity(), currentPeriod).create().show().subscribe(selectedDates -> {
                    if (!selectedDates.isEmpty()) {
                        noSelectedDates(selectedDates, weeklyFormat);

                    } else {
                        selectedDates(selectedDates, weeklyFormat);
                    }
                }, Timber::d);
    ...
    private void onAllOrgUnitClick() {
        if (treeView != null) {
    ...
    private void onUnselectAllOrgUnitClick() {
        if (treeView != null) {
            for (TreeNode node : treeView.getSelected()) {
    ...
    public void addTree(TreeNode treeNode) {
        if (isAdded() && getContext() != null) {
            ...
            binding.orgUnitAll.setOnClickListener(view -> onAllOrgUnitClick());

            binding.orgUnitUnselectAll.setOnClickListener(view -> onUnselectAllOrgUnitClick());
            ...
            treeView.setDefaultNodeClickListener((node, value) -> onDefaultNodeClick(node));

            binding.buttonOrgUnit.setText(String.format(getString(R.string.org_unit_filter), treeView.getSelected().size()));
        }
    }

    private void onDefaultNodeClick(TreeNode node) {
        if (isAdded()) {
