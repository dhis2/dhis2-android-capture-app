...
class ProgramEventDetailRepositoryImpl internal constructor(
    ...
    override fun filteredProgramEvents(textFilter: TextFilter?): LiveData<PagedList<EventViewModel>> {
        val dataSource = filterPresenter
            .filteredEventProgram(program().blockingGet(), textFilter)
            .dataSource
            .map { event: Event? ->
                mapper.eventToEventViewModel(
                    event!!
                )
            }
        return LivePagedListBuilder(object : DataSource.Factory<Event?, EventViewModel>() {
