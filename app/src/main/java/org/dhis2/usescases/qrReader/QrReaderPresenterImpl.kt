    ...
    override fun download() {
                ...
                for (j in 0..<attrArray.length()) {
                    try {
                        ...
                        if (attrValueModel != null) {
                            runBlocking { d2.databaseAdapter().upsertObject(attrValueModel, TrackedEntityAttributeValue::class) }
                        }
                    } catch (e: JSONException) {
                        Timber.e(e)
                    } catch (e: ParseException) {
        ...
        for (i in teiDataJson.indices) {
            try {
                ...
                if (attrValueModel != null) {
                    val result = runBlocking { d2.databaseAdapter().upsertObject(attrValueModel, TrackedEntityDataValue::class) }
                    Log.d("RESULT", "upsert event ${result?.name}")
                }
            } catch (e: JSONException) {
                Timber.e(e)
            } catch (e: ParseException) {
    ...
    override fun onlineDownload() {
        ...
        compositeDisposable.add(
            d2
                ...
                .subscribe(
                    Consumer { data: TrackerD2Progress? -> },
