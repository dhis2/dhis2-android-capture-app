...
class SearchAdapterDiffCallback() {

    companion object {

        @JvmStatic
        fun getDiffCallback(): DiffUtil.ItemCallback<SearchTeiModel> {
            return object : DiffUtil.ItemCallback<SearchTeiModel>() {
                override fun areItemsTheSame(oldItem: SearchTeiModel, newItem: SearchTeiModel) =
                    oldItem.tei?.uid() == newItem.tei?.uid()

                override fun areContentsTheSame(oldItem: SearchTeiModel, newItem: SearchTeiModel) =
                    oldItem.tei?.uid() == newItem.tei?.uid() &&
