...
public class SyncActivity extends ActivityGlobalAbstract implements SyncContracts.View {
    ...
    private void handleDataState(WorkInfo.State dataState) {
        switch (dataState) {
            ...
            case SUCCEEDED:
                ...
                presenter.scheduleSync(provider.sharedPreferences().getInt(Constants.TIME_META, Constants.TIME_DAILY),
                        provider.sharedPreferences().getInt(Constants.TIME_DATA, Constants.TIME_15M));
    ...
    public void startMain() {
        provider.sharedPreferences().putBoolean(Preference.INITIAL_SYNC_DONE.name(), true);
