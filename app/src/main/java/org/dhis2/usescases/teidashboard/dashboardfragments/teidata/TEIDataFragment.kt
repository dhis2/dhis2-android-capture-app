...
//        No riskColor to set
    ...
    override  fun showTeiImage (filePath: String, defaultIcon: String) {
        ...
        if (isPortrait && isImagepathAndDefaultIconAvailable(filePath, defaultIcon)) {
            binding.cardFront?.teiImage?.visibility = View.VISIBLE
            configureImageView(binding.cardFront?.teiImage!!)
        } else {
            binding.cardFront?.teiImage?.visibility = View.GONE
        }

        if (isLandscape && isImagepathAndDefaultIconAvailable(filePath, defaultIcon)) {
            binding.cardFrontLand?.teiImage?.visibility = View.VISIBLE
            configureImageView(binding.cardFrontLand?.teiImage!!)
        } else {
            binding.cardFrontLand?.teiImage?.visibility = View.GONE
        }
    }
