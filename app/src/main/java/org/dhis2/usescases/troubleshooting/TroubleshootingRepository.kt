...
class TroubleshootingRepository(
    ...
    fun validateProgramRules(): List<ProgramRuleValidation> {
        ...
        programRules()
            .mapNotNull { programAndRule ->
                ...
                val ruleActionConditions =
                    rule.actions.mapNotNull { ruleAction ->
    ...
    private fun RuleAction.needsContent() =
        when (this.type) {
    ...
    private fun checkActionVariables(ruleAction: RuleAction): String? =
        when {
            ruleAction.type == ProgramRuleActionType.HIDEFIELD.name &&
                ruleAction
                    .field()
                    ?.isEmpty() == true ->
                "Missing field"
