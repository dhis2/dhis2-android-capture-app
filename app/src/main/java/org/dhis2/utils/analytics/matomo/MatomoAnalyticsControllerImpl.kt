...
class MatomoAnalyticsControllerImpl(
    ...
    private fun updateDhisImplementationTrackerFirstTime() {
        if (dhisImplementationTracker == null &&
            D2Manager.isD2Instantiated() &&
            D2Manager
                .getD2()
                .userModule()
    ...
    override fun trackEventWithDimension(
        ...
        if (isAnalyticsPermissionGranted()) {
            matomoTracker?.let {
                TrackHelper
                    .track()
                    .dimension(index, dimensionValue)
                    .event(category, action)
                    .name(label)
                    .with(it)
            }
        ...
        if (isAnalyticsPermissionGranted()) {
            matomoTracker?.let {
                TrackHelper
                    ...
                    .screen(screen)
                    .title(title)
                    .dimension(firstIndex, firstValue)
                    .dimension(secondIndex, secondValue)
                    .with(it)
            }
            updateDhisImplementationTrackerFirstTime()
            dhisImplementationTracker?.let {
    ...
    override fun updateDhisImplementationTracker(
        ...
        dhisImplementationTracker =
            TrackerController.dhis2ExternalTracker(
                matomoInstance,
                matomoUrl,
                siteId,
                trackerName,
