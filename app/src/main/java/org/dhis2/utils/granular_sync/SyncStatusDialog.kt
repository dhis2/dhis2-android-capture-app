...
class SyncStatusDialog private constructor(private val recordUid: String, private val conflictType: ConflictType,
                                           ...
                                           private val dismissListener: GranularSyncContracts.OnDismissListener?) : BottomSheetDialogFragment(), GranularSyncContracts.View {
    ...
    class Builder {
        ...
        fun build(): SyncStatusDialog {
            if (conflictType == ConflictType.DATA_VALUES && (orgUnitDataValue == null || attributeComboDataValue == null || periodIdDataValue == null))
                throw NullPointerException("DataSets require non null, orgUnit, attributeOptionCombo and periodId")
            return SyncStatusDialog(recordUid, conflictType,
    ...
    override fun setState(state: State) {
        ...
        when (state) {
            State.TO_POST, State.TO_UPDATE -> setNoConflictMessage(getString(R.string.no_conflicts_update_message))
            State.SYNCED -> setNoConflictMessage(getString(R.string.no_conflicts_synced_message))
            State.WARNING, State.ERROR -> if (conflictType == ConflictType.PROGRAM || conflictType == ConflictType.DATA_SET)
    ...
    private fun setProgramConflictMessage(state: State) {
        ...
        val src = getString(if (state == State.WARNING) R.string.data_sync_warning_program else R.string.data_sync_error_program)
        val str = SpannableString(src)
        val wIndex = src.indexOf('@')
        val eIndex = src.indexOf('$')
        if (wIndex > -1)
            str.setSpan(ImageSpan(context!!, R.drawable.ic_sync_warning), wIndex, wIndex + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        if (eIndex > -1)
            str.setSpan(ImageSpan(context!!, R.drawable.ic_sync_problem_red), eIndex, eIndex + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    private fun stateChanged(states: List<SmsSendingService.SendingStatus>?) {
        ...
        for (status in states) {
            when {
                status.state.isSending -> {
                    adapter!!.addItem(StatusLogItem.create(Date(), getString(R.string.sms_bar_state_sending)))
                    adapter!!.addItem(StatusLogItem.create(Date(), status.sent.toString() + "/" + status.total))
                }
                status.state.isError -> adapter!!.addItem(StatusLogItem.create(Date(), getString(R.string.sms_bar_state_failed)))
                status.state.isCompleted -> adapter!!.addItem(StatusLogItem.create(Date(), getString(R.string.sms_bar_state_sent)))
            }
        }
        if (states.isEmpty()) {
            return
        ...
        when (lastState.state!!) {
            ...
            SmsSendingService.State.STARTED, SmsSendingService.State.CONVERTED, SmsSendingService.State.SENDING, SmsSendingService.State.WAITING_RESULT, SmsSendingService.State.RESULT_CONFIRMED, SmsSendingService.State.SENT -> syncing = true
            SmsSendingService.State.ITEM_NOT_READY, SmsSendingService.State.COUNT_NOT_ACCEPTED, SmsSendingService.State.WAITING_RESULT_TIMEOUT, SmsSendingService.State.ERROR, SmsSendingService.State.COMPLETED -> {
            }
        }
    }
    ...
    private fun syncGranular() {
        syncing = true
        presenter!!.initGranularSync().observe(this, Observer { workInfo ->
    ...
    private fun manageWorkInfo(workInfo: WorkInfo) {
        ...
        when (workInfo.state) {
            WorkInfo.State.ENQUEUED -> {
                ...
                adapter!!.addItem(StatusLogItem.create(Calendar.getInstance().time,
                        getString(R.string.start_sync_granular)))
            }
            WorkInfo.State.RUNNING -> adapter!!.addItem(StatusLogItem.create(Calendar.getInstance().time,
            ...
            WorkInfo.State.FAILED -> {
                ...
                if (workInfo.outputData.keyValueMap["conflict"] != null) {
                    for (tracker in workInfo.outputData.getStringArray("conflict")!!) {
                        try {
                            listStatusLog.add(StatusLogItem.create(DateUtils.databaseDateFormat().parse(tracker.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[0]), tracker.split("/".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1]))
            ...
            WorkInfo.State.CANCELLED -> adapter!!.addItem(StatusLogItem.create(Calendar.getInstance().time,
