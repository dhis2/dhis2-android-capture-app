...
class EnrollmentPresenterImplTest {
    ...
    fun `Save value when value is data value, exist the value, the event value is not null, havent the same value, and is not image type`() {
        assert(mockValuesDataElement(existValue = true, eventValueIsNull = false, isSameValue = false, valueTypeIsImage = false) == true)
    }
    ...
    private fun mockValuesDataElement(
            ...
            isSameValue: Boolean,
            valueTypeIsImage: Boolean): Boolean? {

        val uid = "this is the uid"
        val sameValue = "this is the value"
        val value: String? = if (isSameValue) {
            sameValue
        } else {
            "otherValue"
        }
        ...
        whenever(d2.trackedEntityModule().trackedEntityAttributes().uid(uid)) doReturn mock()
        whenever(d2.trackedEntityModule().trackedEntityAttributes().uid(uid).blockingExists()) doReturn false
        ...
        whenever(d2.trackedEntityModule()) doReturn mock()
        whenever(d2.trackedEntityModule().trackedEntityDataValues()) doReturn mock()
        whenever(d2.trackedEntityModule().trackedEntityDataValues().byDataElement()) doReturn mock()
        ...
        whenever(d2.trackedEntityModule().trackedEntityDataValues().byDataElement().eq(uid).byEvent().`in`(listOf())) doReturn  mock()
        if (eventValueIsNull) {
            whenever(d2.trackedEntityModule().trackedEntityDataValues().byDataElement().eq(uid).byEvent().`in`(listOf()).blockingGet()) doReturn listOf()
        } else {
            whenever(d2.trackedEntityModule().trackedEntityDataValues().byDataElement().eq(uid).byEvent().`in`(listOf()).blockingGet()) doReturn listOf(
        ...
        whenever(d2.dataElementModule()) doReturn mock()
        whenever(d2.dataElementModule().dataElements()) doReturn mock()
        ...
        return presenter.saveValue(uid, value)
    }
