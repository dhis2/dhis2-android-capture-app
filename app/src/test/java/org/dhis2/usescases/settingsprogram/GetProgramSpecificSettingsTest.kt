...
class GetProgramSpecificSettingsTest {
    ...
    @Test
    fun `should return a list of specific settings`() = runTest {
        val trackerProgramSetting = ProgramSetting.builder()
        ...
        val eventProgramSetting = ProgramSetting.builder()
            .name("Event Program")
            .uid("eventProgramUid")
            .teiDownload(500)
        ...
        val dataSetProgramSetting = DataSetSetting.builder()
            .name("Data set")
            .uid("dataSetUid")
            .periodDSDownload(12)
            .build()

        val programSpecificSettings = mapOf<String, ProgramSetting>(
            "trackerProgramUid" to trackerProgramSetting,
            "eventProgramUid" to eventProgramSetting,
        )
        val dataSetSpecificSettings = mapOf<String, DataSetSetting>(
            "dataSetUid" to dataSetProgramSetting,
        )

        val programSettings: ProgramSettings = mock {
            on { specificSettings() } doReturn programSpecificSettings
        }

        val datasetSettings: DataSetSettings = mock {
            on { specificSettings() } doReturn dataSetSpecificSettings
        }

        val testingSettings: SynchronizationSettings = mock {
