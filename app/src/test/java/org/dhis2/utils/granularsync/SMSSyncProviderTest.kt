...
class SMSSyncProviderTest {
    ...
    private val smsSender: SmsSubmitCase =
        mock {
    ...
    fun `should return sms enabled if has correct version`() {
        ...
        val result =
            smsSyncProvider(ConflictType.TEI)
                .isSMSEnabled(true)
    ...
    fun smsSyncProvider(conflictType: ConflictType) =
        SMSSyncProviderImpl(
            d2,
            when (conflictType) {
    ...
    fun smsSyncProviderDataValue(conflictType: ConflictType) =
        SMSSyncProviderImpl(
            d2,
            SyncContext.DataSetInstance(
                "uid",
