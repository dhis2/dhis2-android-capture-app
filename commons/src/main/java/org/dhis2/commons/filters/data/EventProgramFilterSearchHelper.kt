...
class EventProgramFilterSearchHelper
    @Inject
    constructor(
        ...
        override fun applyFiltersTo(repository: EventQueryCollectionRepository): EventQueryCollectionRepository =
            repository
        ...
        private fun applyWorkingList(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
        ...
        private fun applyEventStatus(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
        ...
        private fun applyCategoryOptionComboFilter(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
            if (filterManager.catOptComboFilters.isNotEmpty()) {
        ...
        private fun applyOrgUnitFilter(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
            if (filterManager.orgUnitUidsFilters.isNotEmpty()) {
                filterRepository.applyOrgUnitFilter(eventRepository, filterManager.orgUnitUidsFilters)
            } else {
                eventRepository
            }

        private fun applyStateFilter(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
        ...
        private fun applyDateFilter(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
        ...
        private fun applyAssignedToMeFilter(eventRepository: EventQueryCollectionRepository): EventQueryCollectionRepository =
        ...
        override fun applySorting(repository: EventQueryCollectionRepository): EventQueryCollectionRepository =
            filterManager.sortingItem?.let { sortingItem ->
                ...
                orderDirection?.let {
                    when (sortingItem.filterSelectedForSorting) {
                        Filters.PERIOD ->
                            filterRepository.sortByEventDate(
                                repository,
                                orderDirection,
                            )
                        Filters.ORG_UNIT ->
                            filterRepository.sortByOrgUnit(
                                repository,
                                orderDirection,
                            )
                        else -> repository
