...
data class TeiWorkingListScope(
    ...
    override fun isAssignedToMeActive(): Boolean = assignedToMe?.isNotEmpty() == true && assignedToMe.any { it == AssignedUserMode.CURRENT }

    override fun isEnrollmentStatusActive(): Boolean = enrollmentStatusList?.isNotEmpty() == true

    override fun isPeriodActive(filterType: Filters): Boolean =
        when (filterType) {
