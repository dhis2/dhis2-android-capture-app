...
class TrackerFilterSearchHelper
    @Inject
    constructor(
        ...
        override fun applyFiltersTo(repository: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            repository
        ...
        private fun applyWorkingList(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            if (filterManager.workingListActive()) {
                filterRepository
                    .applyWorkingList(
                        teiQuery,
        ...
        private fun applyEnrollmentStatusFilter(
            ...
            if (filterManager.enrollmentStatusFilters.isNotEmpty()) {
        ...
        private fun applyEventStatusFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            if (filterManager.eventStatusFilters.isNotEmpty()) {
                filterRepository
        ...
        private fun applyOrgUnitFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository {
            val orgUnits: MutableList<String> = mutableListOf()
            val ouMode =
                if (filterManager.orgUnitUidsFilters.isEmpty()) {
        ...
        private fun applyStateFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
        ...
        private fun applyDateFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            if (filterManager.periodFilters.isNotEmpty()) {
        ...
        private fun applyEnrollmentDateFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            if (filterManager.enrollmentPeriodFilters.isNotEmpty()) {
        ...
        private fun applyAssignedToMeFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            if (filterManager.assignedFilter) {
                filterRepository.applyAssignToMe(teiQuery)
            } else {
                teiQuery
            }

        private fun applyFollowUpFilter(teiQuery: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            if (filterManager.followUpFilter) {
                filterRepository.applyFollowUp(teiQuery)
            } else {
                teiQuery
            }

        override fun applySorting(repository: TrackedEntitySearchCollectionRepository): TrackedEntitySearchCollectionRepository =
            filterManager.sortingItem?.let { sortingItem ->
                val orderDirection =
                    getSortingDirection(
                ...
                orderDirection?.let {
                    when (sortingItem.filterSelectedForSorting) {
                        ...
                        Filters.ENROLLMENT_DATE ->
                            filterRepository.sortByEnrollmentDate(
                                repository,
                                orderDirection,
                            )

                        Filters.ENROLLMENT_STATUS ->
                            filterRepository.sortByEnrollmentStatus(
                                repository,
                                orderDirection,
                            )
