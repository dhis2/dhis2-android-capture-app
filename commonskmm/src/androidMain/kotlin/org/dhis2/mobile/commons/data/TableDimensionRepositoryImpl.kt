...
class TableDimensionRepositoryImpl(
    ...
    override suspend fun saveWidthForSection(tableId: String, sectionUid: String, widthDpValue: Float) {
    ...
    override suspend fun saveTableWidth(tableId: String, sectionUid: String, widthDpValue: Float) {
        d2.dataStoreModule().localDataStore()
            .value(tableExtraWidthDataStoreKey(dataSetUid, sectionUid, tableId))
            .blockingSet(widthDpValue.toString())
    }

    override suspend fun resetTable(tableId: String, sectionUid: String) {
        d2.dataStoreModule().localDataStore()
            .byKey().like(rowHeaderWidthDataStoreKey(dataSetUid, tableId, sectionUid))
        ...
        d2.dataStoreModule().localDataStore()
            .byKey().like(columnWidthDataStoreKeysForTable(dataSetUid, tableId, sectionUid))
            .blockingGet().forEach { keyValuePair ->
                keyValuePair.key()?.let { key ->
                    clearKey(key)
                }
        ...
        d2.dataStoreModule().localDataStore()
            .byKey().like(tableExtraWidthDataStoreKey(dataSetUid, sectionUid, tableId))
            .blockingGet().forEach { keyValuePair ->
                keyValuePair.key()?.let { key ->
                    clearKey(key)
                }
    ...
    override suspend fun getTableWidth(sectionUid: String): Map<String, Float>? {
        return d2.dataStoreModule().localDataStore()
            .byKey().like(tableExtraWidthForDataSet(dataSetUid, sectionUid))
    ...
    override suspend fun getWidthForSection(sectionUid: String): Map<String, Float>? {
        return d2.dataStoreModule().localDataStore()
            .byKey().like(rowHeaderWidthDataStoreKeyForDataSet(dataSetUid, sectionUid))
            .blockingGet().mapNotNull {
                val key = it.key()
                val metadata = key?.split("_")
                val tableId = metadata?.get(metadata.size - 1)
    ...
    override suspend fun getColumnWidthForSection(
        ...
        return if (tableList.isNotEmpty()) {
    ...
    private fun columnWidthForDataSet(sectionUid: String): Map<String, Map<Int, Float>>? {
        val tableLists = d2.dataStoreModule().localDataStore()
            .byKey().like(columnWidthDataStoreKeysForDataSet(dataSetUid, sectionUid))
            .blockingGet().mapNotNull {
                val key = it.key()
                val metadata = key?.split("_")
