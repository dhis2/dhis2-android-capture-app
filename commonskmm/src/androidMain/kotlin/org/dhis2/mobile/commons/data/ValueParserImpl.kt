...
internal class ValueParserImpl(private val d2: D2) : ValueParser {
    ...
    override suspend fun getValueInfo(uid: String, value: String) =
    ...
    private fun valueAsOptionExists(value: String, optionSetUid: String): Boolean {
    ...
    private fun dataElement(uid: String) = d2.dataElementModule().dataElements()
        .uid(uid)
        .blockingGet()

    private fun valueAsFileExists(value: String): Boolean {
        return d2.fileResourceModule().fileResources().byUid().eq(value).one().blockingExists()
    }

    private fun valueAsOrgUnitExist(value: String): Boolean {
        return d2.organisationUnitModule().organisationUnits().uid(value).blockingExists()
    }
}
