...
fun isBiderectional(): Expression =
    Expression.eq(
        Expression.get(MapRelationshipsToFeatureCollection.BIDIRECTIONAL),
        true,
    )

fun isUnidirectional(): Expression =
    Expression.eq(
        Expression.get(MapRelationshipsToFeatureCollection.BIDIRECTIONAL),
        false,
    )

fun SymbolLayer.withTEIMarkerProperties(): SymbolLayer =
    withProperties(
...
fun SymbolLayer.withInitialVisibility(
...
fun SymbolLayer.withDEIconAndTextProperties(): SymbolLayer =
    withProperties(
