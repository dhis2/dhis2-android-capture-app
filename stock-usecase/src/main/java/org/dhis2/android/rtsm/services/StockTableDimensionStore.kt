...
class StockTableDimensionStore @Inject constructor(
    ...
    fun setUids(programUid: String, sectionUid: String = "") {
    ...
    fun getTableWidth(): Map<String, Float>? {
        return d2.dataStoreModule().localDataStore()
            .byKey().like(tableExtraWidthForProgram())
    ...
    fun getWidthForSection(): Map<String, Float>? {
        return d2.dataStoreModule().localDataStore()
            .byKey().like(rowHeaderWidthDataStoreKeyForProgram())
            .blockingGet().mapNotNull {
                val key = it.key()
                val metadata = key?.split("_")
                val tableId = metadata?.get(metadata.size - 1)
    ...
    private fun columnWidthForProgram(): Map<String, Map<Int, Float>>? {
        val tableLists = d2.dataStoreModule().localDataStore()
            .byKey().like(columnWidthDataStoreKeysForProgram())
            .blockingGet().mapNotNull {
                val key = it.key()
                val metadata = key?.split("_")
    ...
    private fun columnWidthDataStoreKeysForTable(tableId: String) =
        "col_width_${programUid}_${sectionUid}_${tableId}_%"
    ...
    private fun rowHeaderWidthDataStoreKey(tableId: String) =
        "row_width_${programUid}_${sectionUid}_$tableId"
