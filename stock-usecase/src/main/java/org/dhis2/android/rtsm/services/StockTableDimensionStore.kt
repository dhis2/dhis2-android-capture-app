...
class StockTableDimensionStore(
    val d2: D2,
) {
    ...
    fun resetTable(tableId: String) {
        d2
            ...
            .localDataStore()
            .byKey()
            .like(rowHeaderWidthDataStoreKey(tableId))
            .blockingGet()
        ...
        d2
            ...
            .localDataStore()
            .byKey()
            .like(columnWidthDataStoreKeysForTable(tableId))
            .blockingGet()
            .forEach { keyValuePair ->
                keyValuePair.key()?.let { key ->
                    clearKey(key)
        ...
        d2
            ...
            .localDataStore()
            .byKey()
            .like(tableExtraWidthDataStoreKey(tableId))
            .blockingGet()
            .forEach { keyValuePair ->
                keyValuePair.key()?.let { key ->
                    clearKey(key)
    ...
    fun getTableWidth(): Map<String, Float>? =
        d2
            ...
            .localDataStore()
            .byKey()
            .like(tableExtraWidthForProgram())
            .blockingGet()
            .mapNotNull {
                val key = it.key()
    ...
    fun getWidthForSection(): Map<String, Float>? =
        d2
            ...
            .localDataStore()
            .byKey()
            .like(rowHeaderWidthDataStoreKeyForProgram())
            .blockingGet()
            .mapNotNull {
                val key = it.key()
                val metadata = key?.split("_")
    ...
    private fun columnWidthForProgram(): Map<String, Map<Int, Float>>? {
        val tableLists =
            d2
                .dataStoreModule()
                .localDataStore()
                .byKey()
                .like(columnWidthDataStoreKeysForProgram())
                .blockingGet()
                .mapNotNull {
                    val key = it.key()
                    val metadata = key?.split("_")
    ...
    private fun columnWidthDataStoreKeysForProgram() = "col_width_${programUid}_${sectionUid}_%"

    private fun columnWidthDataStoreKeysForTable(tableId: String) = "col_width_${programUid}_${sectionUid}_${tableId}_%"

    private fun rowHeaderWidthDataStoreKeyForProgram() = "row_width_${programUid}_${sectionUid}_%"

    private fun columnWidthDataStoreKey(
        tableId: String,
        column: Int,
    ) = "col_width_${programUid}_${sectionUid}_${tableId}_$column"

    private fun rowHeaderWidthDataStoreKey(tableId: String) = "row_width_${programUid}_${sectionUid}_$tableId"

    private fun tableExtraWidthDataStoreKey(tableId: String) = "table_width_${programUid}_${sectionUid}_$tableId"
